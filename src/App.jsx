...
<div
key={i}
className={w-[4px] h-[4px] rounded-[0.5px] mx-auto transition-all duration-[600ms] ease-in-out   ${isDarkMode   ? (isActive ? 'bg-neutral-400/40' : 'bg-neutral-800/10')   : (isActive ? 'bg-neutral-500/30' : 'bg-neutral-200/30')}}
/>
);
})}
</div>
...
{/* fade edges */}
<div className={absolute left-0 top-0 bottom-0 w-10 z-10 pointer-events-none ${isDarkMode ? 'bg-gradient-to-r from-[#0a0a0a] to-transparent' : 'bg-gradient-to-r from-[#FAFAFA] to-transparent'}} />
<div className={absolute right-0 top-0 bottom-0 w-10 z-10 pointer-events-none ${isDarkMode ? 'bg-gradient-to-l from-[#0a0a0a] to-transparent' : 'bg-gradient-to-l from-[#FAFAFA] to-transparent'}} />
...
<style>{`  
    ...
    <div className="marquee-track">  
      {doubled.map((skill, index) => (  
        <div  
          key={index}  
          className={`flex items-center gap-2.5 mx-2.5 px-4 py-2.5 rounded-full border whitespace-nowrap shrink-0 transition-colors  
            ${isDarkMode  
              ? 'bg-neutral-900 border-neutral-800 text-neutral-300'  
              : 'bg-neutral-100 border-neutral-200 text-neutral-600'}`}  
        >  
          <div className={`shrink-0 ${isDarkMode ? 'text-neutral-400' : 'text-neutral-500'}`}>  
            {skill.icon}  
          </div>  
...
return (
  ...
  >  
    <div className="absolute top-0 inset-x-0 pt-2 [mask-image:linear-gradient(to_bottom,white_60%,transparent)]">  
      <LivePixelMatrix />  
    </div>  <div className="max-w-md mx-auto px-6 pt-16 space-y-10 relative z-10">  
  ...
  {/* Skills — animated marquee, full bleed */}  
</div>  

{/* Skills marquee sits outside the padded container for full-width bleed */}  
<div className="max-w-md mx-auto pt-0 pb-0 relative z-10">  
  <div className="px-6">  
    <SkillsMarquee />  
  </div>  
</div>  

<div className="max-w-md mx-auto px-6 space-y-10 relative z-10 pt-10">  
  {/* Contact */}  
